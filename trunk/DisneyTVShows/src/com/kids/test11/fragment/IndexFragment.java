...
public class IndexFragment extends BaseFragment implements OnPageChangeListener{
...
//	private ArrayList<InfoVideo> dataAds;
	...
	private void init()
	{
		...
		mIndicator = (CirclePageIndicator)root.findViewById(R.id.index_layout_indicator);
		...
		rlContainPager = (RelativeLayout)root.findViewById(R.id.index_layout_viewpager);
		
//		dataAds = new ArrayList<InfoVideo>();
		dataPlaylist = new ArrayList<InfoAlbum>();
		dataVideo = new ArrayList<InfoVideo>();
		
		adapterAds = new SlideShowImageAdapter(getActivity() , imageLoader , options);
...
//		viewPager.setOnPageChangeListener(this);
	...
	public void onWait(BzydroidHttpClient arg0) {
		...
		try {
			...
			JSONArray jArrPlaylist = jObjData.getJSONArray(E4KidsConfig.Home.KEY_PLAYLIST_INDEX);
			
			JSONArray jArrAdsExit = jObjData.getJSONArray(E4KidsConfig.Home.KEY_ADS_EXIT);
			...
			tvTitleVideo.setVisibility(View.VISIBLE);
			  ...
			  adapterVideo = new VideoIndexFragmentAdapter(getActivity(), dataVideo, imageLoader , options);
			...
			for (int i = 0; i < lengthPlaylist; i++) {
				JSONObject objectPla = jArrPlaylist.getJSONObject(i);
			...
			adapterAlbum = new AlbumIndexFragmentAdapter(getActivity(), dataPlaylist, imageLoader , options);
...
//				a
//				Bundle bundle = new Bundle();
//				bundle.putString(E4KidsConfig.KEY_INTENT_AID, id);
//				
//				gotoActivity(ListVideoByPlaActivity.class, bundle);
			    }
			});
